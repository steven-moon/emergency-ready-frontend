    ...
    <base-header class="pb-6 pt-5 container-fluid">
        ...
        <div v-else class="">
            ...
            <ul class="nav nav-tabs row align-items-center text-center pb-2 pt-3">
                ...
                <li class="active col-6 col-md-3 col-xl-3 pb-2">
                    <button @click.prevent="setActiveChart('new-deaths')" class="btn btn-icon btn-max" :class="buttonClass('new-deaths')"  type="button">
                        New Deaths
                    </button>
                </li>
            </ul>
            ...
            <div class="row pt-3"  v-if="$asidebar.activeChart === 'new-deaths'">
                <div class="col-md-6">
                    <stats-card :sub-title="overViewValues.new_deaths"
                                icon="ni ni-satisfied"
                                title="New Deaths"
                                type="gradient-green">

                        <template slot="footer">
                            <span class="text-success mr-2"><i class="fa fa-arrow-up"></i> {{overViewValues.new_deaths_percentage}}%</span>
                            <span class="text-nowrap">Last {{period}} days</span>
                ...
                <div class="col-md-6">
                    <bar-chart :isLoading="isLoading" :customStep="customStep" :activeIndex="4"></bar-chart>
                </div>
            </div>
        </div>
    </base-header>
            ...
            overViewValues() {
                ...
                if (this.totals && this.totals.length > 0) {
                    ...
                    if (this.totals.length >= this.period && this.period > 0) {
                        ...
                        //New Deaths Percentage Increase
                        var last_week_new_deaths = parseInt(last_week.new_deaths);
                        var new_deaths_diff = deaths - last_week_new_deaths;
                        if (new_deaths_diff > 0 && last_week_new_deaths > 0) {
                            new_deaths_percentage = ((new_deaths_diff / last_week_new_deaths) * 100).toFixed(2);
                        }
